  ...
  return (
    <div className="min-h-screen flex flex-col bg-white pb-20">
      ...
      <main className="flex-1 px-4 pt-4 flex flex-col gap-4">
        {/* TODO: OpenWeatherMap API로 교체 */}
        <div className="bg-[#E1F5EE] rounded-2xl px-4 py-3 flex justify-between items-center">
          <div className="flex items-center gap-2">
            <span className="text-lg">☀️</span>
            <span className="text-sm font-medium text-[#085041]">{w.condition} · 오후 2시</span>
          </div>
        ...
        <div className="rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
          <Link href={`/place/${place.id}`}>
            <div className="h-52 bg-[#E1F5EE] flex items-center justify-center relative cursor-pointer">
          ...
          <div className="p-4 space-y-3">
            <Link href={`/place/${place.id}`}>
              <div>
                ...
                <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                  ...
                  <span className={`text-xs font-medium ${place.isOpen ? "text-[#1D9E75]" : "text-red-400"}`}>
            ...
            {/* TODO: 정연 도서 매칭 API로 교체 */}
            <div>
              <p className="text-xs text-gray-400 mb-2">이 공간에 어울리는 책</p>
              <Link href={`/book/${book.id}`}>
                <div className="flex items-center gap-3 p-2.5 rounded-xl bg-gray-50 active:bg-gray-100 transition-colors">
                  <div
                  ...
                  <div>
                    ...
                    <div className="flex gap-1 mt-1">
                      {book.tags.slice(0, 2).map((t) => (
                        <span key={t} className="text-[10px] text-[#3C3489] bg-[#EEEDFE] px-1.5 py-0.5 rounded-full">{t}</span>
                      ))}
                    </div>
                  </div>
                </div>
